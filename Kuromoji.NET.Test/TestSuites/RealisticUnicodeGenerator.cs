...
namespace Kuromoji.NET.Test.TestSuites
{
    class RealisticUnicodeGenerator : StringGenerator
    {
        ...
        /// </summary>
        static readonly int[] blockStarts = new int[] {
        ...
        /// </summary>
        static readonly int[] blockEnds = new int[] {
        ...
        public override string OfCodePointsLength(Random r, int minCodePoints, int maxCodePoints)
        {
            ...
            var block = r.Next(blockStarts.Length);
            var sb = new StringBuilder();
            for (int i = 0; i < length; i++)
            {
                sb.Append((char)r.Next(blockStarts[block], blockEnds[block] + 1));
            }
            return sb.ToString();
        }
        ...
        public override string OfCodeUnitsLength(Random r, int minCodeUnits, int maxCodeUnits)
        {
            ...
            while (length > 0)
            {
                var cp = r.Next(blockStarts[block], blockEnds[block] + 1);
                if (length > CharCount(cp))
                {
                    sb.Append(CodePointToUnit(cp));
                    length -= CharCount(cp);
                }
                else
                {
        ...
        // see: https://stackoverflow.com/a/48873470
        int CharCount(int c) => c >= 0x10000 ? 2 : 1;

        string CodePointToUnit(int c)
        {
            var chars = new char[CharCount(c)];
            switch (chars.Length)
            {
                case 2:
                    chars[0] = (char)((c >> 16) & 0xFFFF);
                    chars[1] = (char)(c & 0xFFFF);
                    break;
                case 1:
                    chars[0] = (char)c;
                    break;
